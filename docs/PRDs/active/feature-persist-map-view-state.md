...
| 2026-07-24 | Deep review of PR #8: 26 verified findings fixed in stacked PR #9 (re-entry catch-up for markers/drawer, unified raid paths with raid-identity reconciliation, invariant-culture settings round-trip, init retry latch, monitoring gates + log-folder re-point, atomic view-state save, e2e serialization/hardening; 76 unit + 9 e2e green). Owner resolved the open semantics question as Option 1 (programmatic raid/screenshot switches count as the last viewed map — behavior kept as implemented) and approved the `ValidateView` → `NormalizeSavedView` rename (applied). Remaining optional follow-up from the review: `CenterMapInView` generation-token hardening. | josephjang |
